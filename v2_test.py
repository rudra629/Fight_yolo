...
print("Booting up the Kinematic Fight Detection Engine (v2 Test)...")
...
# NEW CROWD-CONTROL FILTERS
GLITCH_MAX_SPEED = 120    # If speed is > 120, it's a YOLO overlapping glitch. Ignore it.
MIN_REACTION_SPEED = 5    # The second person must be moving/flinching at least a tiny bit.
    ...
    if results[0].boxes.id is not None:
        ...
        for i, track_id in enumerate(track_ids):
            ...
            if len(kpts) > 10 and kpts[9][0] != 0 and kpts[10][0] != 0:
                ...
                # --- NEW: TELEPORTATION GLITCH FILTER ---
                if max_speed > GLITCH_MAX_SPEED:
                    max_speed = 0 
        ...
        # 2. THE CROWD-SMART COLLISION CHECK
        if len(active_people) >= 2:
            for i in range(len(active_people)):
                for j in range(i + 1, len(active_people)):
                    ...
                    # One person is swinging fast
                    aggressive_movement = (p1['speed'] > VELOCITY_THRESHOLD) or (p2['speed'] > VELOCITY_THRESHOLD)
                    
                    # --- NEW: REACTION GATE ---
                    # Both people must have at least *some* motion (filters out 1 guy talking fast while others stand still)
                    both_reacting = (p1['speed'] > MIN_REACTION_SPEED) and (p2['speed'] > MIN_REACTION_SPEED)
                    
                    if distance < PROXIMITY_THRESHOLD and aggressive_movement and both_reacting:
                        is_fighting = True
                        break 
